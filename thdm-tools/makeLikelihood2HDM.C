...
TGraph *gr21Dspline_tanB(RooSplineND *spline, RooRealVar &ldu, RooRealVar &lVu, RooRealVar &kuu, int type, double minNLL, double fixcbma)
{
	...
	double thmax = 10;
	if (type==1) thmax=50;
	for (double th=0.001; th<=thmax;th+=0.1){

		 double x = fixcbma;
		 double y = (th);  // x irrelevant in grid search

		 if (type==1)type1(x, y, &Vldu, &VlVu, &Vkuu);
		 else if (type==2)type2(x, y, &Vldu, &VlVu, &Vkuu);
          	 ldu.setVal(Vldu);
          	 lVu.setVal(VlVu);
          	 kuu.setVal(Vkuu);
		 ...
		 if ( val < 0 ) val = 0 ;
		 points->SetPoint(pcounter,y,val);
	//	 std::cout << "th " << th << ", " << y << ", deltaNLL, " << val << std::endl; 
		 pcounter++;

		 if (val<mostMin) mostMin=val;
	}

	// Re-norm 
   	for (int p=0;p<points->GetN();p++){
        	double x = (points->GetX())[p];
        	double y = (points->GetY())[p]-mostMin;
		points->SetPoint(p,x,y);
   	}
	
	points->GetXaxis()->SetTitle("tan(#beta)");
	points->GetYaxis()->SetTitle("-2#Delta Log(L)");
	return points;
...
void makeLikelihoodRotation(std::string inname, std::string outname, double SMOOTH, bool isAsimov=false){
   ...
   */
   ...
   TGraph *bf_type1		= new TGraph();  bf_type1->SetName("best_fit_type1"); 
   TGraph *bf_type2		= new TGraph();  bf_type2->SetName("best_fit_type2"); 
   ...
   if (!isAsimov){
    ...
    for (double cbma=-0.9;cbma<0.9;cbma+=0.01){
     for (double b=0.1;b<1.49;b+=0.02){
	...
	if (Vldu > ldu.getMax() || Vldu < ldu.getMin()) {
        	type1_minscan->SetPoint(ccounter,cbma,tanb,50);
	}
	...
	if (Vldu > ldu.getMax() || Vldu < ldu.getMin()) {
        	type2_minscan->SetPoint(ccounter,cbma,tanb,50);
	}
	if (VlVu > lVu.getMax() || VlVu < lVu.getMin()) {
        	type2_minscan->SetPoint(ccounter,cbma,tanb,50);
	}
    ...
    bf_type1->SetPoint(0,mint1_x,mint1_y);
    bf_type2->SetPoint(0,mint2_x,mint2_y);
