...
samples = { 
	"Z(#nu#nu)+jets (Strong)":[ 
	    [
		  pre_mc+"all_zjets/Skim_ZJetsToNuNu_HT-100To200.root"
		 ,pre_mc+"all_zjets/Skim_ZJetsToNuNu_HT-1200To2500.root"
		 ,pre_mc+"all_zjets/Skim_ZJetsToNuNu_HT-200To400.root"
		 ,pre_mc+"all_zjets/Skim_ZJetsToNuNu_HT-2500ToInf.root"
		 ,pre_mc+"all_zjets/Skim_ZJetsToNuNu_HT-400To600.root"
		 ,pre_mc+"all_zjets/Skim_ZJetsToNuNu_HT-600To800.root"
		 ,pre_mc+"all_zjets/Skim_ZJetsToNuNu_HT-800To1200.root"
	    ]
	    , [1,1,1,1,1,1,1]
	    , ROOT.kAzure-9 ]
	,"Z(#nu#nu)+jets (VBF)"	:[ 
	    [
	    	pre_mc+"all_ewkznunu/Skim_EWKZ2Jets_ZToNuNu.root"
	    ]
	    , [1]
	    , ROOT.kRed+7 
	    ]
	,"W(l#nu)+jets (Strong)":[ 
	    [
		 pre_mc+"all_wjets/Skim_WJetsToLNu_HT-100To200.root"
		,pre_mc+"all_wjets/Skim_WJetsToLNu_HT-1200To2500.root"
		,pre_mc+"all_wjets/Skim_WJetsToLNu_HT-200To400.root"
		,pre_mc+"all_wjets/Skim_WJetsToLNu_HT-2500ToInf.root"
		,pre_mc+"all_wjets/Skim_WJetsToLNu_HT-400To600.root"
		,pre_mc+"all_wjets/Skim_WJetsToLNu_HT-600To800.root"
		,pre_mc+"all_wjets/Skim_WJetsToLNu_HT-70To100.root"
		,pre_mc+"all_wjets/Skim_WJetsToLNu_HT-800To1200.root"
	    ]
	    , [1,1,1,1,1,1,1,1]
	    , ROOT.kAzure 
	    ]
	...
	,"Dibosons"		:[ 
	    [
		 pre_mc+"all_vv/Skim_WW.root"
		,pre_mc+"all_vv/Skim_WZ.root"
		,pre_mc+"all_vv/Skim_ZZ.root"
	    ]
	    , [1,1,1]
	    , ROOT.kGray+1 
	    ]
	,"Top"			:[ 
	    [
		 pre_mc+"all_topincl/Skim_ST_s-channel_4f_hadronicDecays.root"
		,pre_mc+"all_topincl/Skim_ST_s-channel_4f_leptonDecays.root"
		,pre_mc+"all_topincl/Skim_ST_t-channel_antitop_4f_inclusiveDecays.root"
		,pre_mc+"all_topincl/Skim_ST_t-channel_top_4f_inclusiveDecays.root"
		,pre_mc+"all_topincl/Skim_ST_tW_antitop_5f_inclusiveDecays.root"
		,pre_mc+"all_topincl/Skim_ST_tW_top_5f_inclusiveDecays.root"
		,pre_mc+"all_topincl/Skim_TTJets_inclusive_NLO_MG.root"
	    ]
	    , [1,1,1,1,1,1,1]
	    , ROOT.kOrange 
	    ]
	,"Z(ll)+jets"		:[ 
	    [
		 pre_mc+"all_dy/Skim_DYJetsToLL_M-50_HT-100to200.root"
		,pre_mc+"all_dy/Skim_DYJetsToLL_M-50_HT-1200to2500.root"
		,pre_mc+"all_dy/Skim_DYJetsToLL_M-50_HT-200to400.root"
		,pre_mc+"all_dy/Skim_DYJetsToLL_M-50_HT-2500toInf.root"
		,pre_mc+"all_dy/Skim_DYJetsToLL_M-50_HT-400to600.root"
	...
	,"QCD"			:[ 
	    [
	        pre_mc+"all_qcd/Skim_QCD_HT1000to1500.root"
	       ,pre_mc+"all_qcd/Skim_QCD_HT100to200.root"
	       ,pre_mc+"all_qcd/Skim_QCD_HT1500to2000.root"
	       ,pre_mc+"all_qcd/Skim_QCD_HT2000toInf.root"
	       ,pre_mc+"all_qcd/Skim_QCD_HT200to300.root"
	       ,pre_mc+"all_qcd/Skim_QCD_HT300to500.root"
	       ,pre_mc+"all_qcd/Skim_QCD_HT500to700.root"
	       ,pre_mc+"all_qcd/Skim_QCD_HT700to1000.root"
	    ]
	    , [1,1,1,1,1,1,1,1]
	    , ROOT.kOrange+6 
	    ]
...
,	   "dijet_M_an_highpTrel"	  :["m_{jj} (GeV)","BINS",9,[200,400,600,900,1200,1500,2000,2750,3500,5000],True,0,[],[],[]] 
...
def selectVTR(tr):
     ...
     if (not (tr.HLT_DiJet110_35_Mjj650_PFMET110 > 0.1 or tr.HLT_TripleJet110_35_35_Mjj650_PFMET110 > 0.1) ): return False 
     if ( abs(tr.lMjj_jet1_eta) <= 2.5 ): 
       if not (( tr.lMjj_jet1_chHEF > 0.1 ) & ( tr.lMjj_jet1_neHEF < 0.8 )): return False
     if ( abs(tr.lMjj_jet2_eta) <= 2.5 ): 
       if not (( tr.lMjj_jet2_chHEF > 0.1 ) & ( tr.lMjj_jet2_neHEF < 0.8 )): return False
     return True
...
def selectMTR(tr): 
     ...
     if (not (tr.HLT_PFMETNoMu120_PFMHTNoMu120_IDTight>0.1 or tr.HLT_PFMETNoMu120_PFMHTNoMu120_IDTight_PFHT60>0.1) ): return False 
     if ( abs(tr.Leading_jet_eta) <= 2.5 ): 
       if not (( tr.Leading_jet_chHEF > 0.1 ) & ( tr.Leading_jet_neHEF < 0.8 )): return False
     if ( abs(tr.Subleading_jet_eta) <= 2.5 ): 
       if not (( tr.Subleading_jet_chHEF > 0.1 ) & ( tr.Subleading_jet_neHEF < 0.8 )): return False
     return True
...
def doAnalysis(tr,entry,i,w):
     ...
     jet1    = ROOT.TVector3();
     jet2    = ROOT.TVector3();
     jet14vec    = ROOT.TLorentzVector();
     jet24vec    = ROOT.TLorentzVector();
     ...
     if selectVTR(tr): 
       ...
       jet14vec.SetPtEtaPhiM(tr.lMjj_jet1_pt,tr.lMjj_jet1_eta,tr.lMjj_jet1_phi,0)
       jet24vec.SetPtEtaPhiM(tr.lMjj_jet2_pt,tr.lMjj_jet2_eta,tr.lMjj_jet2_phi,0)

       dijet = jet1+jet2
       dijet4vec = jet14vec+jet24vec
       #dijet4vec.SetPtEtaPhiM(dijet.Pt(),dijet.Eta(),dijet.Phi(),dijet4)
       ...
       jet14vec.SetPtEtaPhiM(tr.Leading_jet_pt,tr.Leading_jet_eta,tr.Leading_jet_phi,0)
       jet24vec.SetPtEtaPhiM(tr.Subleading_jet_pt,tr.Subleading_jet_eta,tr.Subleading_jet_phi,0)
     ...
     if tr.isData==1 : 
       w=1
        ...
        if passSel == "VTR": 
            w = tr.trigger_weight_VBF2018*tr.puWeight*tr.xs_weight*tr.VLooseTauFix_eventVetoW*tr.VetoElectron_eventVetoW*tr.LooseMuon_eventVetoW*L
            w *= tr.fnlo_SF_EWK_corr*tr.fnlo_SF_QCD_corr_QCD_proc_VTR*tr.fnlo_SF_QCD_corr_EWK_proc
        else: 
            w = tr.trigger_weight_METMHT2018*tr.puWeight*tr.xs_weight*tr.VLooseTauFix_eventVetoW*tr.VetoElectron_eventVetoW*tr.LooseMuon_eventVetoW*L
     ...
     if passSel=="VTR":
      if abs(tr.JetMetmindPhi) < 1.8 : Fill(passSel+"dijet_pT_reldiff_MET_QCDCR",ptRelBalance,w)
     else:
      if abs(tr.JetMetmindPhi) < 0.5 : Fill(passSel+"dijet_pT_reldiff_MET_QCDCR",ptRelBalance,w)


     if passSel=="VTR":
       if abs(tr.JetMetmindPhi) < 1.8 : return 0
     else : 
	...
	if ObjectIsInHEM(sj[1]):
     ...
     Fill(passSel+"dijet_M_an",dijet4vec.M(),w)
     Fill(passSel+"dijet_M_bins",dijet4vec.M(),w)
